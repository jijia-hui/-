...
- **说明**：两张图——① 领域模型类图（对应数据库表结构）；② 后端设计类图（Django/DRF 分层实现）。对象级顺序图（OBJ-SEQ01~15）在后续阶段按用例逐一补充。
...
| assignments | id, course_id, title, description, test_cases(JSON), deadline, reference_file, created_at | N-1 课程；1-N 提交 |
